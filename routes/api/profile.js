...
router.get('/', passport.authenticate('jwt', { session: false }), (req,res) => {
    ...
    Profile.findOne({ user: req.user.id }).populate('user', ['name','avatar']).then(profile => {
        if (!profile){
            errors.noprofile = "No se encontró el perfil";
...
router.get('/handle/:handle', (req,res) => {
    ...
    Profile.findOne({handle: req.params.handle}).populate('user', ['name','avatar']).then(profile => {
        if (!profile){
            errors.noprofile = 'No existe el perfil';
...
router.get('/user/:user_id', (req,res) => {
    ...
    Profile.findOne({user: req.params.user_id}).populate('user', ['name','avatar']).then(profile => {
        ...
        res.json(profile);
    }).catch(err => res.status(404).json({profile: "No existe el perfil"}));
});
...
router.get('/all',(req,res) =>{
    ...
    Profile.find().populate('user', ['name','avatar']).then(profiles => {
        ...
        res.json(profiles);
    }).catch(err => res.status(404).json({noprofile: "No hay perfiles"}));
});
...
// @route  POST api/profile/experiencie
// @desc   Add experiencie to profile
// @access Private
